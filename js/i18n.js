/* ============================================================
   i18n – EN / BG dictionary + instant switcher
   ============================================================ */

const I18N = {
  en: {
    "nav.about": "About",
    "droid.hint": "hover to boot ✦",
    "term.hint": "paint over me ✦",
    "ag.hint": "– hover me",
    "term.hintTap": "drag across me ✦",
    "droid.hintTap": "tap to boot ✦",
    "nav.expertise": "Expertise",
    "nav.journey": "Journey",
    "nav.newsletter": "Newsletter",
    "nl.label": "04 – Newsletter",
    "nl.eyebrow": "// weekly, on LinkedIn",
    "nl.sub": "AI in practice",
    "nl.desc": "Practical thinking on artificial intelligence, business, education and the world that is coming.",
    "nl.subs": "subscribers",
    "nl.issues": "editions",
    "nl.cta": "Subscribe on LinkedIn",
    "nl.all": "all editions ↗",
    "nav.appearances": "Appearances",
    "nav.contact": "Contact",
    "nav.gallery": "Gallery ✦",
    "nav.notebook": "AI Notebook ✎",

    "hero.kicker": "// AI Integration Strategist & Educational Expert",
    "hero.sub": "I help companies and educators turn AI from hype into",
    "hero.cta1": "Let's talk",
    "hero.cta2": "Explore my work",
    "hero.scroll": "scroll",
    "hero.worldwide": "working worldwide",

    "about.label": "01 – About",
    "about.title": "Technology is the tool. People are the point.",
    "about.p1": "For more than a decade I've lived at the intersection of education and technology – teaching over 12,500 students, directing programmes at Bulgaria's leading tech academies, and building TeachMe as CEO and co-founder.",
    "about.p2": "Today I help organisations integrate AI the way it actually works: strategically, ethically, and with people at the centre. Not chasing tools – building capability.",
    "about.p3": "Outside of work I'm a father, a passionate cook and a relentless reader. The best systems I know – classrooms, companies, families – all run on the same thing: trust.",

    "exp.label": "02 – Expertise",
    "exp.title": "What I bring to the table",
    "exp.c1t": "AI Integration Strategy",
    "exp.c1d": "Audits, roadmaps and hands-on guidance that take AI from a slide deck into your daily operations.",
    "exp.c2t": "AI in Education",
    "exp.c2d": "Curriculum design and AI-powered learning experiences that make educators faster and learners stronger.",
    "exp.c3t": "Technical Training",
    "exp.c3d": "Workshops and live trainings for teams – from prompt literacy to building with LLM APIs.",
    "exp.c4t": "Product Development",
    "exp.c4d": "From idea to launch – I've built and scaled educational products used by thousands of people.",

    "stats.s1": "students taught",
    "stats.s2": "years in education",
    "stats.s3": "live lessons",
    "stats.s4": "media features",

    "jr.label": "03 – Journey",
    "jr.title": "The road so far",
    "jr.now": "now",
    "jr.now2": "now",
    "jr.t1r": "Educational Director",
    "jr.t1d": "Led programmes at Bulgaria's largest software university.",
    "jr.t2r": "CEO & Co-founder",
    "jr.t2d": "Building a platform that connects learners with the right teachers.",
    "jr.t3r": "Educational Director",
    "jr.t3d": "Directed creative-tech education for game art & design.",
    "jr.t4t": "AI Integration",
    "jr.t4r": "Strategist & Consultant",
    "jr.t4d": "Helping organisations adopt AI with strategy, training and measurable impact.",
    "jr.now3": "now",
    "jr.t5r": "Educational Director",
    "jr.t5d": "Leading award-winning, AI-first engineering education at Sirma Group.",

    "ap.label": "05 – Appearances",
    "ap.i1": "What we learned while teaching others",
    "ap.i2": "AI is not taking jobs, it is rearranging them",
    "ap.i3": "AI Tools Introduction – Practical AI",
    "ap.i4": "Sirma Academy and career development",
    "ap.i5": "AI looks more and more like a big bubble",
    "ap.i6": "Sirma Academy launches practical AI courses",
    "ap.i7": "Getting into the IT industry in 2025",
    "ap.i8": "Teachers are the guiding light of every nation",
    "ap.i9": "Software engineering is the maker of the future",
    "ap.i10": "What matters most if you want to become a programmer",
    "ap.i11": "Which software technologies are popular?",
    "ap.i12": "Conversations about the future",
    "ap.i13": "A learning platform brings private lessons into the open",
    "ap.i14": "Programming for beginners with C# – live lesson",
    "ap.i15": "How AI is changing professions and what new roles companies look for",
    "ap.i16": "The next question is not who uses AI, but who uses it more wisely",
    "ap.i17": "A race with no profit: the US and China bet everything on AI",
    "ap.i18": "ARC Online – the new online game development courses",
    "ap.i19": "The IT world and the new full software engineering programme",
    "ap.i20": "Excel automation – saving time and resources",
    "ap.i21": "Programming for beginners with C# – introduction to programming",
    "ap.i22": "Microsoft Excel for beginners – working with tables",
    "ap.i23": "Microsoft Excel for beginners – working with text",
    "ap.i24": "Programming for beginners with C# – methods",
    "ap.i25": "Programming for beginners with C# – conditionals",
    "ap.i26": "Programming for beginners with C# – conditionals (exercise)",
    "ap.i27": "Programming for beginners with C# – loops",
    "ap.i28": "Programming for beginners with C# – loops (exercise)",
    "ap.title": "Talks, media & lectures",

    "ct.label": "06 – Contact",
    "ct.title1": "Let's build",
    "ct.title2": "what's next.",

    "footer.note": "designed & built with curiosity",
    "footer.beforeAi": "my website before AI ↗",

    rotator: ["real strategy", "better learning", "shipped products", "human advantage"]
  },

  bg: {
    "nav.about": "За мен",
    "droid.hint": "мини с мишката ✦",
    "term.hint": "нарисувай ме ✦",
    "ag.hint": "– мини отгоре",
    "term.hintTap": "плъзни по мен ✦",
    "droid.hintTap": "докосни за старт ✦",
    "nav.expertise": "Експертиза",
    "nav.journey": "Път",
    "nav.newsletter": "Бюлетин",
    "nl.label": "04 – Бюлетин",
    "nl.eyebrow": "// седмично, в LinkedIn",
    "nl.sub": "AI на практика",
    "nl.desc": "Практически идеи за изкуствен интелект, бизнес, образование и света, който предстои.",
    "nl.subs": "абонати",
    "nl.issues": "броя",
    "nl.cta": "Абонирай се в LinkedIn",
    "nl.all": "всички броеве ↗",
    "nav.appearances": "Участия",
    "nav.contact": "Контакт",
    "nav.gallery": "Галерия ✦",
    "nav.notebook": "AI Тетрадката ✎",

    "hero.kicker": "// AI стратег по интеграция & експерт в образованието",
    "hero.sub": "Помагам на компании и преподаватели да превърнат AI от мода в",
    "hero.cta1": "Свържи се",
    "hero.cta2": "Виж работата ми",
    "hero.scroll": "скрол",
    "hero.worldwide": "работя по цял свят",

    "about.label": "01 – За мен",
    "about.title": "Технологията е инструментът. Хората са смисълът.",
    "about.p1": "Повече от десетилетие живея на пресечната точка между образование и технологии – преподавал съм на над 12 500 ученици, ръководил съм програми във водещите технологични академии в България и изграждам TeachMe като CEO и съосновател.",
    "about.p2": "Днес помагам на организации да интегрират AI така, както наистина работи: стратегически, етично и с хората в центъра. Не гонене на инструменти – изграждане на способности.",
    "about.p3": "Извън работата съм баща, страстен готвач и неуморен читател. Най-добрите системи, които познавам – класни стаи, компании, семейства – работят на едно и също гориво: доверие.",

    "exp.label": "02 – Експертиза",
    "exp.title": "Какво нося със себе си",
    "exp.c1t": "AI интеграционна стратегия",
    "exp.c1d": "Одити, пътни карти и практическо водене, които пренасят AI от презентацията в ежедневната ви работа.",
    "exp.c2t": "AI в образованието",
    "exp.c2d": "Дизайн на учебни програми и AI-базирани учебни преживявания, които правят преподавателите по-бързи, а учащите – по-силни.",
    "exp.c3t": "Технически обучения",
    "exp.c3d": "Уъркшопи и обучения на живо за екипи – от грамотност с промптове до разработка с LLM API-та.",
    "exp.c4t": "Разработка на продукти",
    "exp.c4d": "От идея до пазар – изграждал съм и съм скалирал образователни продукти, ползвани от хиляди хора.",

    "stats.s1": "обучени ученици",
    "stats.s2": "години в образованието",
    "stats.s3": "уроци на живо",
    "stats.s4": "медийни участия",

    "jr.label": "03 – Път",
    "jr.title": "Пътят дотук",
    "jr.now": "сега",
    "jr.now2": "сега",
    "jr.t1r": "Директор „Образование“",
    "jr.t1d": "Ръководих програмите на най-големия софтуерен университет в България.",
    "jr.t2r": "CEO и съосновател",
    "jr.t2d": "Изграждам платформа, която свързва учащите с правилните преподаватели.",
    "jr.t3r": "Директор „Образование“",
    "jr.t3d": "Ръководих креативно-техническото образование за гейм арт и дизайн.",
    "jr.t4t": "AI интеграция",
    "jr.t4r": "Стратег и консултант",
    "jr.t4d": "Помагам на организации да въведат AI със стратегия, обучения и измерим ефект.",
    "jr.now3": "сега",
    "jr.t5r": "Директор „Образование“",
    "jr.t5d": "Ръководя награждаваното AI-first инженерно обучение в Sirma Group.",

    "ap.label": "05 – Участия",
    "ap.i1": "Какво научихме, докато обучавахме другите?",
    "ap.i2": "AI не отнема работни места, той ги пренарежда",
    "ap.i3": "AI Tools Introduction – Practical AI",
    "ap.i4": "Sirma Academy и кариерното развитие",
    "ap.i5": "AI все повече прилича на голям балон",
    "ap.i6": "Sirma Academy стартира практически AI курсове",
    "ap.i7": "Вход в IT индустрията през 2025",
    "ap.i8": "Учителите са пътеводната светлина на всяка нация",
    "ap.i9": "Софтуерното инженерство е създателят на бъдещето",
    "ap.i10": "Най-важното, за да станеш програмист",
    "ap.i11": "Кои са популярните софтуерни технологии?",
    "ap.i12": "Разговори за бъдещето",
    "ap.i13": "Платформа за обучение изкарва частните уроци на светло",
    "ap.i14": "Програмиране за начинаещи с C# – урок на живо",
    "ap.i15": "Как AI променя професиите и какви нови роли търсят компаниите",
    "ap.i16": "Следващият въпрос няма да е кой използва AI, а кой го използва по-умно",
    "ap.i17": "Надпревара без печалба: САЩ и Китай залагат всичко на AI",
    "ap.i18": "ARC Online – новите онлайн курсове по game development",
    "ap.i19": "IT светът и новата цялостна програма по софтуерно инженерство",
    "ap.i20": "Автоматизация на Excel – спестяване на време и ресурси",
    "ap.i21": "Програмиране за начинаещи с C# – въведение в програмирането",
    "ap.i22": "Microsoft Excel за начинаещи – работа с таблици",
    "ap.i23": "Microsoft Excel за начинаещи – работа с текст",
    "ap.i24": "Програмиране за начинаещи с C# – методи",
    "ap.i25": "Програмиране за начинаещи с C# – условни конструкции",
    "ap.i26": "Програмиране за начинаещи с C# – условни конструкции (упражнение)",
    "ap.i27": "Програмиране за начинаещи с C# – цикли",
    "ap.i28": "Програмиране за начинаещи с C# – цикли (упражнение)",
    "ap.title": "Лекции, медии и участия",

    "ct.label": "06 – Контакт",
    "ct.title1": "Да създадем",
    "ct.title2": "бъдещето.",

    "footer.note": "проектиран и създаден с любопитство",
    "footer.beforeAi": "сайтът ми преди AI ↗",

    rotator: ["реална стратегия", "по-добро учене", "завършени продукти", "човешко предимство"]
  }
};

let currentLang = localStorage.getItem("lang") || "en";

function applyLang(lang) {
  currentLang = lang;
  localStorage.setItem("lang", lang);
  document.documentElement.lang = lang;

  const dict = I18N[lang];
  document.querySelectorAll("[data-i18n]").forEach((el) => {
    const key = el.getAttribute("data-i18n");
    if (dict[key] !== undefined) el.textContent = dict[key];
  });

  document.querySelectorAll(".lang-toggle__opt").forEach((opt) => {
    opt.classList.toggle("is-active", opt.dataset.lang === lang);
  });

  // notify rotator (main.js) that the word list changed
  document.dispatchEvent(new CustomEvent("langchange", { detail: { lang } }));
}

document.addEventListener("DOMContentLoaded", () => {
  applyLang(currentLang);
  document.getElementById("langToggle").addEventListener("click", () => {
    applyLang(currentLang === "en" ? "bg" : "en");
  });
});
